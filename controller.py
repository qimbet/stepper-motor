...
j_mattie@live.ca
https://github.com/qimbet/stepper-motor
...
The function halfStep() increments the output on the associated
pins in a way that drives a unipolar stepper motor driver:
...
step() increments the motor by one step. 

The functions moveNumSteps and moveAngle are wrappers on the step() function,
but ultimately do the same thing.

The ordering of the list coilPins is most easily determined through use of the 
paired script: motor_debug.py

Record the resultant list of motor_debug.py and enter it as the value for coilPins.

Add/remove windings as needed.
...
def main():
    ...
    print(f"""Parameters:
Degrees moved per step: {anglePerMotorStep}
Delay between motor steps: {delayBetweenMotorSteps}

Pin output sequence: {coilPins}
          """)
    ...
    while True:
        ...
        currentState = moveAngle(angle, currentState, anglePerMotorStep, delayVal)
